...
#if 0
const double pendriveMaxForehandSpeed[6] = {12.0, 12.0, 20.0, 15.0, 12.0, 12.0};
#else
const double pendriveMaxForehandSpeed[6] = {12.0, 12.0, 20.0, 13.0, 12.0, 12.0};
#endif
const double pendriveMaxBackhandSpeed[6] = {8.0, 8.0, 10.0, 8.0, 8.0, 8.0};
const double pendriveDiffCoeff[6] = {1.0, 1.2, 1.0, 0.8, 1.2, 1.2};

/**
 * Default constructor. 
 * Set player type to pen drive. 
 */
PenDrive::PenDrive() {
  ...
  MAX_FOREHAND_SPEED = (double *)pendriveMaxForehandSpeed;
  MAX_BACKHAND_SPEED = (double *)pendriveMaxBackhandSpeed;
  FOREHAND_BOUNCE_RATE = 0.4;
  BACKHAND_BOUNCE_RATE = 0.4;
  FOREHAND_SPINEFFECT_RATE = 3.0;
  BACKHAND_SPINEFFECT_RATE = 4.0;

  DIFF_COEFF = (double *)pendriveDiffCoeff;
}
 ...
 */
PenDrive::PenDrive( long playerType, long side, const vector3d x,
...
PenDrive::SwingType( Ball *ball, long spin ) {
  ...
  if (canHitBall(ball)) {
    ...
    } else if ( ball->GetX()[2] < TABLEHEIGHT+NETHEIGHT*2 ) { // under the net
      if ( ForeOrBack() ) {
	...
	if ( ball->GetSpin()[1] < 0 ) {
	  ...
	  m_spin[1] = 0.4;
		...
		ball->GetX()[2] > TABLEHEIGHT+NETHEIGHT && ForeOrBack() ) {
