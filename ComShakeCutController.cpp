/**
 * @file
 * @brief Implementation of ComShakeCutController class. 
 ...
 * Default constructor. 
 */
ComShakeCutController::ComShakeCutController() : ComController() {
  HOMEPOSITIONY = -(TABLELENGTH/2+1.0);
}
 ...
 */
ComShakeCutController::ComShakeCutController(Player *parent) : ComController(parent) {
  m_parent = parent;
...
ComShakeCutController::Think() {
  ...
  if ( theBall.GetStatus() == 8 || _hitX[0]*m_parent->GetSide() > 0 ||
 ...
 */
bool
ComShakeCutController::Hitarea( vector2d &hitX ) {
  Ball *tmpBall;
  ...
  if (isOpponentHit(&theBall)) {
	...
	if ( tmpBall->GetX()[2] > max ) {
	  max = tmpBall->GetX()[2];
	  if ( tmpBall->GetX()[1] < TABLELENGTH/2 ) {
	    bestX = tmpBall->GetX()[0];
	    bestY = tmpBall->GetX()[1];
	  }
	}
	if ( tmpBall->GetV()[2] < 0.0 && tmpBall->GetX()[1] > TABLELENGTH/2 &&
	     tmpBall->GetX()[2] > TABLEHEIGHT ) {
	  bestX = tmpBall->GetX()[0];
	  bestY = tmpBall->GetX()[1];
	}
      }
 ...
 */
bool
ComShakeCutController::SetTargetX( Player *opponent ) {
  double width;
